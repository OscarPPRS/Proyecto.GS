...
BEGIN TRY
	...
	DECLARE @CANTIDAD_INTRODUCIDA INT;
		...
		VALUES(@COD_PALET_SIGUIENTE,@COD_REFERENCIA, @UBICACION_DISPONIBLE, @COD_ESTADO, GETDATE(), @CANTIDAD_INTRODUCIDA, @ALBARAN, 'aA');

		SET @MENSAJE = 'PALET CREADO.'

		EXEC PA_GENERAR_ETIQUETA @COD_REFERENCIA, @COD_PALET_SIGUIENTE, @CANTIDAD_INTRODUCIDA, @INVOKER, @RETCODE, @MENSAJE
		...
		BEGIN
			...
			@CANTIDAD_INPUT,@ALBARAN , @INVOKER, @RETCODE, @MENSAJE 

		END
	END
			
	IF @INVOKER = 1 -- MODIFICAR PALET
	...
	BEGIN	
		...
		BEGIN			
			...
			EXEC PA_PALETS @COD_REFERENCIA, @COD_ESTADO, 
			@CANTIDAD_INPUT,@ALBARAN , 0, @RETCODE, @MENSAJE
